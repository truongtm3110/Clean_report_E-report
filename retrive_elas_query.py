...
def load_category_tree(platform: str = 'shopee') -> dict | None:
    with open(f'categories_data/{platform}_categories.json', 'r', encoding='utf-8') as f:
...
def add_aggregations_to_raw_query(raw_query: dict, end_date, platform: str = 'shopee'):
    month_start = '202201'
    ...
    if platform == 'tiktok':
        month_start = '202209'
    lst_months = get_all_month_between(month_start, month_end[0:6])
    field_types = ['revenue', 'sale']
    for month in lst_months:
        for field_type in field_types:
            start_field, end_field = get_end_field_and_start_field(month,end_date, field_type)
...
def get_raw_elasticsearch_query(search_body: dict,end_date, pgn: str, platform: str = 'shopee', output_file_name: str = None):
    """
        ...
        try:
            ...
            if response_status == 200:
                ...
                if not raw_search_body:
                    return None
                els_query = add_aggregations_to_raw_query(raw_search_body,end_date, platform)
                ...
                with open(f'C:/Users/Admin/PycharmProjects/Clean_report_E-report/data_raw_cate/{file_name}.json',
...
def retrieve_elasticsearch_query(dataframe: pd.DataFrame, output_file_name: str):
    ...
    for idx, row in dataframe.iterrows():
        ...
        print("shopee_categories", shopee_categories)
        ...
        if shopee_categories != 'skip':
            ...
            shopee_raw_query = get_raw_elasticsearch_query(shopee_query,end_date, group_name, 'shopee',
        ...
        if lazada_categories != 'skip':
            lazada_query = build_request_body('lazada',
                                              ...
                                              end_date,
                                              duration_days)
            lazada_raw_query = get_raw_elasticsearch_query(lazada_query,end_date, group_name, 'lazada',
                                                           output_file_name=output_file_name)
        ...
        if tiki_categories != 'skip':
            tiki_query = build_request_body('tiki',
                                            ...
                                            end_date,
                                            duration_days)
            tiki_raw_query = get_raw_elasticsearch_query(tiki_query, end_date, group_name, 'tiki',
                                                         output_file_name=output_file_name)
        ...
        if sendo_categories != 'skip':
            sendo_query = build_request_body('sendo',
                                             ...
                                             end_date,
                                             duration_days)
            sendo_raw_query = get_raw_elasticsearch_query(sendo_query,end_date, group_name, 'sendo',
                                                          output_file_name=output_file_name)
        ...
        if tiktok_categories != 'skip':
            tiktok_query = build_request_body('tiktok',
                                              ...
                                              end_date,
                                              duration_days)
            tiktok_raw_query = get_raw_elasticsearch_query(tiktok_query,end_date, group_name, 'tiktok',
                                                           output_file_name=output_file_name)
