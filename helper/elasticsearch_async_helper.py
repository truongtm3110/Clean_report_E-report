...
async def insert_doc(es, index, body, id=None, verbose=True):
    res = await es.create(index=index, id=id, body=body)
    print(res)
...
def insert_docs_in_bulk_with_raw_body(es, action_data_bulk, verbose=True):
    ...
    res = None
    try:
        res = helpers.bulk(es, action_data_bulk, raise_on_error=True)
        if verbose:
            print(res)
...
async def search_v2(es, index_name, query, include_score=False, filter_path=None,
