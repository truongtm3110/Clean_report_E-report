...
def insert_doc(es: Elasticsearch, index, body, id=None, verbose=True):
    res = es.index(index=index, id=id, document=body)
...
def search_v2(es, index_name, query, include_score=False, filter_path=None, timeout: Union[int, float, None] = None):
    response = None
    try:
        if filter_path is None:
            response = es.search(index=index_name, body=query, request_timeout=timeout)
